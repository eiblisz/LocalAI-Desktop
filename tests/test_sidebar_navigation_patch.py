...
from app.sidebar_controller import (
    SidebarController,
    is_schedule_chat,
    open_schedule_history,
)
...
def test_main_window_delegates_sidebar_runtime_to_explicit_controller():
    init_source = inspect.getsource(MainWindow.__init__)
    sidebar_source = inspect.getsource(MainWindow._build_sidebar)
    list_source = inspect.getsource(MainWindow._load_chat_list)
    schedule_source = inspect.getsource(MainWindow._refresh_schedule_task_labels)

    assert "self.sidebar_controller = SidebarController(self)" in init_source
    assert "self.sidebar_controller.build_sidebar()" in sidebar_source
    assert "self.sidebar_controller.load_chat_list()" in list_source
    assert "self.sidebar_controller.refresh_schedule_task_labels()" in schedule_source


def test_sidebar_has_navigation_then_distinct_schedule_and_chat_sections():
    source = inspect.getsource(SidebarController.build_sidebar)

    assert 'QPushButton("+  NEW CHAT")' in source
    assert 'QPushButton("SCHEDULE")' in source
    assert '["PROJECTS", "BROWSER", "MEMORY", "EXTENSIONS", "SETTINGS"]' in source
    assert 'button.clicked.connect(window._open_market_browser)' in source
    assert 'button.clicked.connect(window._open_extensions)' in source
...
def test_schedule_chat_is_excluded_by_task_chat_id_or_schedule_title():
    ...
    assert is_schedule_chat(window, {"id": "schedule-chat-1", "title": "History"})
    assert is_schedule_chat(window, {"id": "orphan", "title": "[SCHEDULE] Old task"})
    assert not is_schedule_chat(window, {"id": "normal-1", "title": "Teaskanna kereses"})


def test_chat_list_filter_is_presentation_only_and_does_not_delete_history():
    source = inspect.getsource(SidebarController.load_chat_list)

    assert "include_closed=True" in source
    assert "is_schedule_chat(window, chat)" in source
...
def test_schedule_rows_are_clickable_and_preserve_compact_style():
    ...
    assert "open_schedule_history" in source
...
def test_schedule_click_opens_existing_history_without_mutating_storage():
    ...
    open_schedule_history(window, "task-1")
...
def test_sidebar_primary_and_navigation_buttons_share_compact_height():
    ...
    assert "window.schedule_button.setFixedHeight(34)" in source
...
def test_sidebar_controls_and_chat_list_share_side_menu_geometry():
    ...
    assert 'window.schedule_button.setObjectName("sideMenuButton")' in source
    assert 'button.setObjectName("sideMenuButton")' in source
    assert 'window.chat_list.setObjectName("sideChatList")' in source
...
def test_memory_navigation_is_enabled_and_opens_canonical_dialog():
    ...
    assert 'button.clicked.connect(window._open_memory)' in source
